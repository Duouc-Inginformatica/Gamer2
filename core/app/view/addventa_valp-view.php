...
$entrada_producto=0; 
$entradas = ProcesoVenta_valpData::getAllEntradas($te->id_producto);
if(count($entradas)>0){ 
...
$salida_producto=0; 
$salidas = ProcesoVenta_valpData::getAllSalidas($te->id_producto);
if(count($salidas)>0){
...
$producto = Product_valpData::getById($te->id_producto);
...
if(count($_POST)>0 and $contador=='0'){
	...
	foreach($tmps as $p): 
		...
		$procesoventa = new ProcesoVenta_valpData();
		$procesoventa->id_producto=$p->id_producto; 
